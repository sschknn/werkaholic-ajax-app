...
export const usePerformanceMonitoring = () => {
  ...
  const measureWebVitals = useCallback(() => {
    ...
    const observerFID = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        performanceData.current.webVitals.FID = fidEntry.processingStart - fidEntry.startTime;
  ...
  const measureLoadTime = useCallback(() => {
    const navigation = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;
    if (navigation) {
      performanceData.current.loadTime = navigation.loadEventEnd - navigation.startTime;
